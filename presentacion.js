...
const dato = document.getElementById("dato_random");
const boton = document.getElementById("nuevo_dato");
...
function iniciarPagina() {
    const pantalla = document.getElementById("pantalla_inicio");
    if (pantalla === null) {
        return;
    }
    setTimeout(function () {
...
const nombreArtista = document.getElementById("nombre_artista");
...
const cancionesLista = document.querySelectorAll(".cancion_lista");
let cancionActual = 0;
...
cancionesLista.forEach(function (cancion) {
    cancion.addEventListener("click", function () {
        const indice = Number(cancion.getAttribute("data-cancion"));
...
function formatearTiempo(segundos) {
    ...
    const segundosTexto = segundosRestantes < 10
        ? "0" + segundosRestantes
